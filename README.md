...
*Figure 1: 3D Potential energy landscape mapping ($U/k_B T$) and lateral interference vector fields computed for customizable multi-trap arrays.*
...
4. **Holographic Array Digital Bench:** Renders the target diffraction arrangement, the generated phase map distributed across the SLM pixels, and a dynamic $3\text{D}$ projection of output optical intensity.
...
* **Gradient Force ($\mathbf{F}_{grad}$):** Pulls the dielectric specimen toward the highest intensity focal point, scaled by the Clausius-Mossotti polarizability ($\alpha$):
  ...
  Where $m = n_p / n_m$ is the relative refractive index ratio between the particle and the medium.
...
Where spatial damping ($\gamma$) and the thermal diffusion coefficient ($D$) obey strict Einstein-Stokes relations:
...
* **Mie Regime Component (Mode B):** Injects large cellular structures ($a > \lambda$) into the same environment. Users can directly witness Mode B specimens being blown out of the trap by dominant radiation scattering forces while Rayleigh structures remain trapped.
